...
public sealed partial class ServerApi : IPostInjectInit
{
    ...
    [Dependency] private readonly IAdminManager _admin = default!; // Frontier: ISharedAdminManager<IAdminManager>
    ...
    private async Task ActionEndGameRule(IStatusHandlerContext context, Actor actor)
    {
        ...
        await RunOnMainThread(async () =>
        {
            ...
            var gameRule = ticker
                .GetActiveGameRules()
                .FirstOrNull(rule =>
                    _entityManager.GetComponent<MetaDataComponent>(rule).EntityPrototype?.ID == body.GameRuleId);
    ...
    private async Task InfoHandler(IStatusHandlerContext context, Actor actor)
    {
        ...
        */
        ...
        var info = await RunOnMainThread<InfoResponse>(() =>
        {
            ...
            {
                var meta = _entityManager.GetComponent<MetaDataComponent>(addedGameRule);
